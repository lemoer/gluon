...
need_site_string('hostname_prefix', false)
need_site_string 'timezone'

need_string_array('ntp_servers', false)

need_domain_string_match('prefix6', '^[%x:]+/64$')


for _, config in ipairs({'wifi24', 'wifi5'}) do
	if need_table(config, nil, false) then
		need_site_string('regdom') -- regdom is only required when wifi24 or wifi5 is configured

		need_number(config .. '.channel')

		local rates = {1000, 2000, 5500, 6000, 9000, 11000, 12000, 18000, 24000, 36000, 48000, 54000}
		local supported_rates = need_site_array_of(config .. '.supported_rates', rates, false)
...
need_site_boolean('poe_passthrough', false)
...
if need_table('next_node', nil, false) then
	need_domain_string_match('next_node.ip6', '^[%x:]+$', false)
	need_domain_string_match('next_node.ip4', '^%d+.%d+.%d+.%d+$', false)
end

for _, config in ipairs({'wifi24', 'wifi5'}) do
  local rates = {1000, 2000, 5500, 6000, 9000, 11000, 12000, 18000, 24000, 36000, 48000, 54000}
  rates = need_site_array_of(config .. '.supported_rates', rates, false) or rates

  if need_table(config .. '.ibss', nil, false) then
    need_domain_string(config .. '.ibss.ssid')
    need_domain_string_match(config .. '.ibss.bssid', '^%x[02468aAcCeE]:%x%x:%x%x:%x%x:%x%x:%x%x$')
  ...
  if need_table(config .. '.mesh', nil, false) then
    need_domain_string(config .. '.mesh.id')
    need_one_of(config .. '.mesh.mcast_rate', rates, false)
    need_boolean(config .. '.mesh.disabled', false)
  end
end

need_site_boolean('mesh_on_wan', false)
need_site_boolean('mesh_on_lan', false)
need_site_boolean('single_as_lan', false)
